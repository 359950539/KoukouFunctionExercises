...
\end{que}
\sol $3-f(x)=g(t)=\dfrac{\sin t}{e^{|t|}}$，$t\in[-4,4]$。$g(t)$为奇函数，故最大值最小值之和为$0$，故$(3-p)+(3-q)=0\Leftrightarrow\res{p+q=6}$。\par\hfill\tg{奇偶性}\easy
...
\par\hfill\tg{交点\ 圆}\easy

\begin{que}
	已知等式$(x^2+2x+2)^5=a_0+a_1(x+1)+\cdots+a_{10}(x+1)^{10}$。求$\displaystyle\sum_{i=1}^{10}a_n$和$\displaystyle\sum_{i=1}^{10}na_n$的值。
\end{que}
\sol 代入$x=-1$知$a_0=1$；代入$x=0$知，$\displaystyle\sum_{i=1}^{10}a_n=2^5-1=\res{31}$；关于$x$求导后代入$x=0$知$\displaystyle\sum_{i=1}^{10}na_n=\res{320}$。\par\hfill\tg{二项式定理}\easy
...
\begin{que}
	...
	\end{enumerate}
\end{que}
\sol\sidenote{此处统一采用记号：$$\binom{n}{k}=C_{n}^k$$这一符号应用更广泛。} \begin{enumerate}
...
\sol \begin{enumerate}
	...
	\end{enumerate}由归纳原理知$a_n=f(3^n)=2\cdot 3^n$，进而\sidenote{也可归纳证明$a_{n+1}=3a_n$。}
...
\end{enumerate}\par\hfill\tg{抽象函数\ 不等式}\normal\par
...
\end{enumerate}\par\hfill\tg{抽象函数\ 不等式}\easy
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{求取参变量范围的综合题目}
这类题目与不等式相关题目有可观的一部分来源于泰勒展开式，简要介绍之。
\begin{thm}
	\blue{\heiti 泰勒级数}\ 若$f(x)$在$D$上有定义，$x_0\in D$，$f(x)$在$D$上无穷次可导，那末$f(x)$可写为
...
\end{thm}
\begin{kaiti}
	\heiti{证明}\ 限于高中知识范围，这里只求系数，不证明存在性，足够使用。\par
...
\begin{que}
	...
	\end{enumerate}
\end{que}
\sol \begin{enumerate}\begin{marginfigure}
	\includegraphics{Figure-10.eps}
...
\end{marginfigure}
	\item $f(x)=0\ \Leftrightarrow\ x=1,3$，$f(2)=1$，我们可大致画出$f(x)$的图像如图，$f(x)=a$可能有2,3,4个根，分别对应$a\in\{0\}\cup(1,+\infty)$，$a=1$，$a\in(0,1)$。所给方程有7个根，意味着$y^2+by+c=0$两根分别满足$x_1=1,\ x_2\in(0,1)$，因此$b=-(x_1+x_2)=-1-x_2\in\res{(-1,0)}$。\begin{marginfigure}
		\includegraphics{Figure-11.eps}
		\caption{题目\arabic{no}第2小题中$f(x)$的图像}
	\end{marginfigure}
	...
	\end{enumerate}综上所述，$\res{m\in(0,1)}$。\begin{marginfigure}
		\includegraphics{Figure-12.eps}
		\caption{题目\arabic{no}第3小题中$f(x)$的图像}
	\end{marginfigure}
	...
	\end{enumerate}
\end{enumerate}\par\hfill\gk\tg{取值范围\ 复合函数}\easy
...
\end{enumerate}\par\hfill\gk\tg{取值范围}\easy
...
\begin{que}
	...
	\end{enumerate}
\end{que}
\sol\sidenote{应当注意到，该函数为$\ln(1+x)$与其泰勒展开式截取前$n$的差，刻画了近似程度。} \begin{enumerate}
	...
	其中\sidenote{本式前三行是多余的，但前三行其实具有提示了一类不等式证明方法的实际意义。}相仿地，当$n=2m$时，$$\begin{aligned}|f_{2m}(1)|&=\left(\ln 2-1+\dfrac{1}{2}\right)-\left(\dfrac{1}{2}-\dfrac{1}{4}\right)-\cdots-\left(\dfrac{1}{2m-2}-\dfrac{1}{2m}\right)\\&<\dfrac{1}{2}-\left(\dfrac{1}{2}-\dfrac{1}{4}\right)-\cdots-\left(\dfrac{1}{2m-2}-\dfrac{1}{2m}\right)\\&=\dfrac{1}{2m}\end{aligned}$$因此$\forall\ n,\ |f_n(1)|<\dfrac{1}{n}$，这就说明了充分性\sidenote{事实上，即使在实数域上，$\alpha$依然最大取到$1$。这说明了$\displaystyle\sum_{k=1}^n\dfrac{(-1)^{k+1}}{k}$趋向于$\ln 2$时误差减小的速度与反比函数同数量级，若以此式计算$\ln 2$的约值，想要精确到小数点后第三位，需要计算万项以上——这决不是一个好主意。我们有更好的逼近方法。}。综上所述，$\alpha$最大取$\res{\alpha=1}$。
\end{enumerate}\par\hfill\tg{取值范围\ 误差分析}\easy
...
\begin{thm}
	\labeltext[\green{［IEQ\ 2］}]{\blue{\heiti{［IEQ\ 2］}}\ ${\ln(1+x)\leqslant x}\ (x>-1),\ {\ln(1+x)\geqslant x-\dfrac{x^2}{2}}\ (x\geqslant 0)$}{ieq2}
\end{thm}
\heiti{证明}\ \begin{kaiti}该式截取自$\ln(1+x)$的麦克劳林展开式$$\ln(1+x)=x-\dfrac{x^2}{2}+\dfrac{x^3}{3}-\cdots+(-1)^{n+1}\dfrac{x^n}{n}+\cdots$$前两项，截取更多项时证明相仿，只需多求几次导数，不赘述，这里我们证第二式。\par
	...
	当仅当$x=0$时取等。\end{kaiti}
\begin{thm}
	\labeltext[\green{［IEQ\ 3］}]{\blue{\heiti{［IEQ\ 3］}}\ ${\ln x \leqslant 1-\dfrac{1}{x}}$}{ieq3}
\end{thm}
\heiti{证明}\ \begin{kaiti}令$g(x)=\ln x-1+\dfrac{1}{x}$，可证$g(x)\geqslant g(1)=0$，因此$\forall x\in \mathbb{R}^+,\ \ln x\geqslant 1-\dfrac{1}{x}$。\end{kaiti}
\begin{thm}
	\labeltext[\green{［IEQ\ 4］}]{\blue{\heiti{［IEQ\ 4］}}\ ${(1+x)^a\geqslant 1+ax}\ (a>1,\ x\geqslant 0)$，${(1+x)^a\leqslant 1+ax}\ (a\neq 0,\ a\leqslant 1,\ x\geqslant 0)$}{ieq4}\par
	\labeltext[\green{［IEQ\ 5］}]{\blue{\heiti{［IEQ\ 5］}}\ ${\ln x\geqslant \dfrac{2(x-1)}{(x+1)}}\ (x\geqslant 1)$}{ieq5}
\end{thm}
\heiti{证明}\ \begin{kaiti}证法同上，略。（逐渐敷衍(*/ω＼*)……然而确实很简单）\end{kaiti}\vspace{1em}
...
\sol \begin{enumerate}
	...
	$$\begin{aligned}
		...
		&=\ln(n+1)-\ln 1=\ln(n+1)
	\end{aligned}$$\par\hfill\tg{不等式}\easy\par
	\begin{kaiti}
...
\end{que}
% \sol \begin{enumerate}
% 	\item $f(x)$在$(0,1)$上单调减，在$[1,+\infty)$上单调增。
% 	\item 由增减性，$b_n=f(n)=\ln(1+n)-n$，因此$a_n=n$，\begin{enumerate}
% 		\item 由题意$$\forall\ n\in\mathbb{N}^+,\ c<n+2-\sqrt{n(n+2)}=t-\sqrt{t^2-1}-1=\dfrac{1}{t+\sqrt{t^2-1}}-1=g(t),\ t=n+1\geqslant 2$$$\vspace{0.5em}g'(t)=1-\dfrac{t}{\sqrt{t^2-1}}<0$，故$\displaystyle c\leqslant \min_{t\in\mathbb{Z}^{\geqslant 2}}g(t)=g(+\infty)=\res{-1}$，由于$g(t)$无法取到最小值，故本式中使用「$\leqslant$」符号。
% 		\item 注意到$\ln(1+x)<x,x\neq 0$，若设$$\dfrac{a_1a_3\cdots a_{2n-1}}{a_2 a_4\cdots a_{2n}}=\left(1-\dfrac{1}{2}\right)\left(1-\dfrac{1}{4}\right)\left(1-\dfrac{1}{6}\right)\cdots\left(1-\dfrac{1}{2n}\right)=k_n$$则$$\begin{aligned}\ln k_n&=\ln\left(1-\dfrac{1}{2}\right)+\ln\left(1-\dfrac{1}{4}\right)+\cdots+\ln\left(1-\dfrac{1}{2n}\right)\\&<-\dfrac{1}{2}\left(1+\dfrac{1}{2}+\cdots+\dfrac{1}{n}\right)<-\dfrac{1}{2}\ln(n+1)=\ln\dfrac{1}{\sqrt{n+1}}\end{aligned}$$\sidenote{此处根据定积分定义得到：$$1+\dfrac{1}{2}+\cdots+\dfrac{1}{n}>\int_1^{n+1}\dfrac{1}{x}\text{d}x=\ln(1+x)$$}又注意到$$\sqrt{2n+1}-\sqrt{2n-1}=\dfrac{2}{\sqrt{2n+1}+\sqrt{2n-1}}>\dfrac{2}{2\sqrt{\dfrac{(2n+1)+(2n-1)}{2}}}=\dfrac{1}{\sqrt{2n}}$$，因此$$\begin{aligned}
% 			\text{原式左\ }&=\dfrac{1}{2}+\dfrac{1\times 3}{2\times 4}+\cdots+\dfrac{1\times 3\times\cdots\times (2n-1)}{2\times 4\times\cdots\times(2n)}\\
% 			&=k_1+k_2+\cdots+k_n<\dfrac{1}{\sqrt{1+1}}+\dfrac{1}{\sqrt{2+1}}+\cdots+\dfrac{1}{\sqrt{n+1}}\\&<\dfrac{1}{2\times 1}+\dfrac{1}{\sqrt{2\times 2}}
% 		\end{aligned}$$
% 	\end{enumerate}
% \end{enumerate}
\par\hfill\gk\tg{不等式\ 数列\ 取值范围}\easy\par
...
\sol \begin{enumerate}
	...
	\item \blue{（方法一）}按照题设要求，给定$[a,b]$，应有其上$g'(x)\leqslant 0$，取定未知$k$，应有$\forall\ t>k$，$g'(a),\ g'(b)<0$，则$$g'(x)=0\Rightarrow x_1=\ln\dfrac{2}{t+\sqrt{t^2-8}}\searrow ,\ x_2=\ln\dfrac{t+\sqrt{t^2-8}}{4}\nearrow ,\ x_2>x_1$$
	...
	\blue{（方法二）}由$g'(x)=2e^{2x}-te^x+1$，我们只需找到$k$，使得$[a,b]$上$$\forall\ t>k,\ g'(x)<0\ \Leftrightarrow\ t>2e^x+e^{-x}=h(x)$$由于$h(x)$在闭区间$[a,b]$上连续，故必能取得最大值\sidenote{关于单变量函数的定理：闭区间上的连续函数存在最值，且必可取到最值。此处闭区间可延伸为闭集。}$M$，取$k=M$即可。
...
\end{enumerate}\par\hfill\gk\tg{不等式}\easy\par
...
\sol \begin{enumerate}
	...
	\end{enumerate}
\end{enumerate}\par\hfill\gk\tg{不等式}\hard\par
...
\sol \begin{enumerate}
	...
	\end{aligned}$$
	% 而$g'(x)=\dfrac{1}{x+1}+2ax-1$，$g(0)=g'(0)=0$，$g''(x)=-\dfrac{1}{(x+1)^2}+2a$，$g''(0)=2a-1$，故首先\sidenote{这种微观的手法前面有\ref{diff}已经使用过，其细节本题及其他所有题目不再赘述，但在试题中应当写清楚。}$g''(0)\leqslant 0$，$\Rightarrow a\leqslant \dfrac{1}{2}$\sidenote{应当注意到，$$\ln(1+x)\geqslant x-\dfrac{x^2}{2}$$形式上是$\ln(1+x)$泰勒展开式截取了前两项，这也是题目来源。}。
\end{enumerate}\par\hfill\tg{取值范围\ 不等式}\easy\par
...
\end{enumerate}\par\hfill\tg{不等式}\easy\par

\section{与割线斜率和函数凸性相关的综合题目}
\begin{thm}
...
\end{thm}
\begin{kaiti}
	\heiti{证明\ \ }设$$F(x)=[f(a)-f(b)][g(x)-g(b)]-[g(a)-g(b)][f(x)-f(b)]$$易知$F(x)$在$[a,b]$上连续，在$(a,b)$上可导，且$F(a)=F(b)=0$，那末由\href{http://course.shufe.edu.cn/gdsx/upfiles/file/20160307155320.pdf}{罗尔(Rolle)定理}\sidenote{这要从柯西列开始写了，能写三四页( ﾟДﾟ)。因为都是非常初等的定理，请自行翻阅链接材料吧。}可知，$\exists\ \xi\in(a,b),\ \text{s.t.\ }F'(\xi)$，稍作整理即得原式。
\end{kaiti}\par
...
\end{enumerate}\par\hfill\tg{取值范围\ 拉格朗日中值定理}\easy
...
\end{enumerate}\par\hfill\tg{拉格朗日中值定理}\easy
...
\sol \begin{enumerate}
	...
	\item $b=0$时，$h(x)=f(x)+g(x)=\ln x+\dfrac{a}{x^2}-1$，$h(1)=a-1\geqslant 0\Rightarrow a\geqslant 1$，。$a\geqslant 1$时，在$[1,2]$上，$$h'(x)=\dfrac{1}{x}-\dfrac{2a}{x^2}=\dfrac{x-2a}{x^2}\leqslant 0\ \Rightarrow\ h(2)=\ln 2+\dfrac{a}{4}-1\geqslant 0\ \Rightarrow\ \res{a\geqslant 4-4\ln 2}$$	\begin{marginfigure}
		...
		\label{fig8}
	\end{marginfigure}
	\item $a=0$时，再记$h(x)=f(x)-g(x)=\ln x-bx+1$，那末「有两个不同的实数解」\sidenote{这一结论可以结合图像得到（不严谨）、可以由\ref{ieq2}得到，也可以由$h\left(\dfrac{1}{b}\right)>0$得到，不赘述。此时$x_1,x_2$并不是相互独立的，并且都是依赖于$b$的函数，$x_1=x_1(b),x_2=x_2(b)$}意味着$0<b<1$。进而由于易知$h(x)$在$\left(0,\dfrac{1}{b}\right)$上单调增，在$\left(\dfrac{1}{b},+\infty\right)$上单调减，必有$x_1\in\left(0,\dfrac{1}{b}\right),x_2\in\left(\dfrac{1}{b},+\infty\right)$。我们在$\left(0,\dfrac{1}{b}\right)$定义一个这样的函数：$$m(t)=h\left(\dfrac{2}{b}-t\right)-h(t)\quad\Rightarrow\quad m'(t)=\dfrac{2(bt-1)^2}{t(bt-2)}<0$$由\ref{fig8}，这一函数的几何意义非常明显\sidenote{本题来源于这一事实：$h(x)$在最高点左侧变化得比最高点右侧快。}，根据$m(t)$单调性和$h(x_1)=h(x_2)=0$可以得到$$m(x_1)=h\underbrace{\left(\dfrac{2}{b}-x_1\right)}_{>1/b}-\underbrace{h(x_1)}_{=h(x_2)}>m\left(\dfrac{2}{b}\right)=0\ \Rightarrow\ h\left(\dfrac{2}{b}\right)>h(x_2)$$因此由$h(x)$单调性必有$x_2>\dfrac{2}{b}-x_1\Rightarrow \res{x_1+x_2>\dfrac{2}{b}>2}$。
	
\end{enumerate}\par\hfill\tg{取值范围\ 相关变量}\normal
...
\end{enumerate}\par\hfill\gk\tg{切线}\easy
...
\sol \begin{enumerate}
	...
	\end{aligned}$$
\end{enumerate}\par\hfill\tg{不等式\ 拉格朗日中值定理}\easy
